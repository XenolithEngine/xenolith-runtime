...
#if defined(__GNUC__) || defined(__clang__) // Any GNU-compatible compiler supporting C++11 has
...
inline bitcount_t flog2(uint32_t v) {
	...
	static const uint8_t multiplyDeBruijnBitPos[32] = {0, 9, 1, 10, 13, 21, 2, 29, 11, 14, 16, 18,
	...
	return multiplyDeBruijnBitPos[(uint32_t)(v * 0x07C4'ACDDU) >> 27];
}

inline bitcount_t trailingzeros(uint32_t v) {
	static const uint8_t multiplyDeBruijnBitPos[32] = {0, 1, 28, 2, 29, 14, 24, 3, 30, 22, 20, 15,
		25, 17, 4, 8, 31, 27, 13, 23, 21, 19, 16, 7, 26, 12, 18, 6, 11, 5, 10, 9};

	return multiplyDeBruijnBitPos[((uint32_t)((v & -v) * 0x077C'B531U)) >> 27];
}
...
class uint_x4 {
	// private:
	static constexpr unsigned int UINT_BITS = sizeof(UInt) * __SPRT_CHAR_BIT;
...
public:
	...
	template <class Integral,
			typename sprt::enable_if<(sprt::is_integral<Integral>::value
	...
	template <typename U, typename V>
	friend sprt::pair< uint_x4<U, V>, uint_x4<U, V> > divmod(const uint_x4<U, V> &,
			const uint_x4<U, V> &);
...
template <typename UInt, typename UIntX2>
sprt::pair< uint_x4<UInt, UIntX2>, uint_x4<UInt, UIntX2> > divmod(
...
uint_x4<UInt32, uint64_t> operator>>(const uint_x4<UInt32, uint64_t> &v, const bitcount_t shift) {
	...
	if (shift >= bits2) {
		...
		return {v.d.v23 >> shift,
			shift ? (v.d.v01 >> shift) | (v.d.v23 << (bits2 - shift)) : v.d.v01};
	}
}
#endif
