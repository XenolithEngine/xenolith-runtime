...
SrcIter uneven_copy_impl(SrcIter src_first, DestIter dest_first, DestIter dest_last,
		sprt::true_type) {
	typedef typename sprt::iterator_traits<SrcIter>::value_type src_t;
	typedef typename sprt::iterator_traits<DestIter>::value_type dest_t;
...
inline SrcIter uneven_copy(SrcIter src_first, DestIter dest_first, DestIter dest_last) {
	...
	return uneven_copy_impl(src_first, dest_first, dest_last,
			sprt::integral_constant<bool, DEST_IS_SMALLER>{});
}
...
inline void generate_to_impl(SeedSeq &&generator, DestIter dest, sprt::true_type) {
	generator.generate(dest, dest + size);
}

template <size_t size, typename SeedSeq, typename DestIter>
void generate_to_impl(SeedSeq &&generator, DestIter dest, sprt::false_type) {
	typedef typename sprt::iterator_traits<DestIter>::value_type dest_t;
	constexpr auto DEST_SIZE = sizeof(dest_t);
...
inline void generate_to(SeedSeq &&generator, DestIter dest) {
	...
	generate_to_impl<size>(sprt::forward<SeedSeq>(generator), dest,
			sprt::integral_constant<bool, IS_32BIT>{});
}
...
inline UInt generate_one(SeedSeq &&generator) {
	UInt result[N];
	generate_to<N>(sprt::forward<SeedSeq>(generator), result);
	return result[i];
}
...
void shuffle(Iter from, Iter to, RandType &&rng) {
	typedef typename sprt::iterator_traits<Iter>::difference_type delta_t;
	typedef typename sprt::remove_reference<RandType>::type::result_type result_t;
	auto count = to - from;
	while (count > 1) {
...
public:
	...
	seed_seq_from(Args &&...args) : rng_(sprt::forward<Args>(args)...) {
		// Nothing (else) to do...
	}
...
public:
	static constexpr IntType value =
			fnv(IntType(2'166'136'261U ^ sizeof(IntType)), __DATE__ __TIME__ __FILE__);
};
