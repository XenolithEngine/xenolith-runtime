...
using uintptr_t = sprt::uintptr_t;
...
PCG_DEFINE_CONSTANT(pcg128_t, default, multiplier,
		PCG_128BIT_CONSTANT(2'549'297'995'355'413'924ULL, 4'865'540'595'714'422'341ULL))
PCG_DEFINE_CONSTANT(pcg128_t, default, increment,
		PCG_128BIT_CONSTANT(6'364'136'223'846'793'005ULL, 1'442'695'040'888'963'407ULL))
...
struct cheap_multiplier<pcg128_t> {
	static constexpr uint64_t multiplier() { return 0xda94'2042'e4dd'58b5ULL; }
};
...
public:
	...
	// It would be nice to use sprt::numeric_limits for these, but
...
public:
	...
	engine(itype state = itype(0xcafe'f00d'd15e'a5e5ULL))
	...
	engine(SeedSeq &&seedSeq,
			typename sprt::enable_if< !stream_mixin::can_specify_stream
							&& !sprt::is_convertible<SeedSeq, itype>::value
							&& !sprt::is_convertible<SeedSeq, engine>::value,
					no_specifiable_stream_tag>::type = {})
	: engine(generate_one<itype>(sprt::forward<SeedSeq>(seedSeq))) {
		// Nothing else to do.
	}
	...
	engine(SeedSeq &&seedSeq,
		...
		itype seeddata[2];
		generate_to<2>(sprt::forward<SeedSeq>(seedSeq), seeddata);
	...
	void seed(Args &&...args) {
		new (this, sprt::nothrow) engine(sprt::forward<Args>(args)...);
	}
...
itype operator-(const engine<xtype, itype, output_mixin, output_previous, stream_mixin_lhs,
	...
	static_assert(sprt::is_same<stream_mixin_lhs, stream_mixin_rhs>::value
					&& sprt::is_same<multiplier_mixin_lhs, multiplier_mixin_rhs>::value,
...
using oneseq_base = engine<xtype, itype, output_mixin<xtype, itype>, output_previous,
		oneseq_stream<itype>, multiplier_mixin<itype> >;

template <typename xtype, typename itype, template <typename XT, typename IT> class output_mixin,
		bool output_previous = (sizeof(itype) <= 8),
		template <typename IT> class multiplier_mixin = default_multiplier>
using unique_base = engine<xtype, itype, output_mixin<xtype, itype>, output_previous,
		unique_stream<itype>, multiplier_mixin<itype> >;

template <typename xtype, typename itype, template <typename XT, typename IT> class output_mixin,
		bool output_previous = (sizeof(itype) <= 8),
		template <typename IT> class multiplier_mixin = default_multiplier>
using setseq_base = engine<xtype, itype, output_mixin<xtype, itype>, output_previous,
...
PCG_DEFINE_CONSTANT(pcg128_t, mcg, multiplier,
		PCG_128BIT_CONSTANT(17'766'728'186'571'221'404ULL, 12'605'985'483'714'917'081ULL))
PCG_DEFINE_CONSTANT(pcg128_t, mcg, unmultiplier,
		PCG_128BIT_CONSTANT(14'422'606'686'972'528'997ULL, 15'009'553'638'781'119'849ULL))
...
public:
	template <typename SeedSeq,
			typename = typename sprt::enable_if< !sprt::is_convertible<SeedSeq, result_type>::value
					&& !sprt::is_convertible<SeedSeq, extended>::value>::type>
	...
	template <typename... Args>
	void seed(Args &&...args) {
		new (this) extended(sprt::forward<Args>(args)...);
	}
...
bool operator==(const extended<table_pow2, advance_pow2, baseclass, extvalclass, kdd> &lhs,
	...
	return base_lhs == base_rhs
			&& sprt::equal(sprt::begin(lhs.data_), sprt::end(lhs.data_), sprt::begin(rhs.data_));
}
