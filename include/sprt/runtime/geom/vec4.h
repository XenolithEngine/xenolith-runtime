...
#ifndef RUNTIME_INCLUDE_SPRT_RUNTIME_GEOM_VEC4_H
#define RUNTIME_INCLUDE_SPRT_RUNTIME_GEOM_VEC4_H
...
class SPRT_API alignas(16) Vec4 {
public:
	...
	static constexpr sprt::bitset<4> bitop(const Vec4 &v, const Functor &f) {
		sprt::bitset<4> ret;
		ret.set(0, f(v.x));
		ret.set(1, f(v.y));
		ret.set(2, f(v.z));
	...
	template <typename Functor>
	static constexpr sprt::bitset<4> bitop(const Vec4 &v1, const Vec4 &v2, const Functor &f) {
		sprt::bitset<4> ret;
	...
	constexpr bool isValid() const { return !isnan(x) && !isnan(y) && !isnan(z) && !isnan(w); }
	...
	constexpr float distance(const Vec4 &v) const { return sprt::sqrt(distanceSquared(v)); }
...
#endif /* RUNTIME_INCLUDE_SPRT_RUNTIME_GEOM_VEC4_H */
