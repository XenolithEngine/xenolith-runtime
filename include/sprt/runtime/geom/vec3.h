...
#ifndef RUNTIME_INCLUDE_SPRT_RUNTIME_GEOM_VEC3_H_
#define RUNTIME_INCLUDE_SPRT_RUNTIME_GEOM_VEC3_H_
...
public:
	...
	static constexpr sprt::bitset<3> bitop(const Vec3 &v, const Functor &f) {
		sprt::bitset<3> ret;
		ret.set(0, f(v.x));
		ret.set(1, f(v.y));
		ret.set(2, f(v.z));
	...
	template <typename Functor>
	static constexpr sprt::bitset<3> bitop(const Vec3 &v1, const Vec3 &v2, const Functor &f) {
		sprt::bitset<3> ret;
	...
	: x(buf.size() > 0 ? buf[0] : NaN<float>)
	, y(buf.size() > 1 ? buf[1] : NaN<float>)
	, z(buf.size() > 2 ? buf[2] : NaN<float>) { }

	explicit Vec3(const Size3 &);
	explicit Vec3(const Extent3 &);

	constexpr bool isValid() const { return !isnan(x) && !isnan(y) && !isnan(z); }
	...
	constexpr float distance(const Vec3 &other) const { return sprt::sqrt(distanceSquared(other)); }
...
constexpr const Vec3 Vec3::INVALID(NaN<float>, NaN<float>, NaN<float>);
...
#endif /* RUNTIME_INCLUDE_SPRT_RUNTIME_GEOM_VEC3_H_ */
