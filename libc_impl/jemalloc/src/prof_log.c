...
static size_t prof_log_bt_index(tsd_t *tsd, prof_bt_t *bt) {
	...
	if (ckh_search(&log_bt_node_set, (void *)(&dummy_node), (void **)(&node), NULL)) {
		...
		prof_bt_node_t *new_node = (prof_bt_node_t *)iallocztm(tsd_tsdn(tsd), sz, sz_size2index(sz),
...
static size_t prof_log_thr_index(tsd_t *tsd, uint64_t thr_uid, const char *name) {
	...
	if (ckh_search(&log_thr_node_set, (void *)(&dummy_node), (void **)(&node), NULL)) {
		...
		prof_thr_node_t *new_node = (prof_thr_node_t *)iallocztm(tsd_tsdn(tsd), sz,
				sz_size2index(sz), false, NULL, true, arena_get(TSDN_NULL, 0, true), true);
...
void prof_try_log(tsd_t *tsd, size_t usize, prof_info_t *prof_info) {
	...
	if (!log_tables_initialized) {
		bool err1 = ckh_new(tsd, &log_bt_node_set, PROF_CKH_MINITEMS, prof_bt_node_hash,
				prof_bt_node_keycomp);
		bool err2 = ckh_new(tsd, &log_thr_node_set, PROF_CKH_MINITEMS, prof_thr_node_hash,
				prof_thr_node_keycomp);
		if (err1 || err2) {
			goto label_done;
		}
	...
	prof_alloc_node_t *new_node = (prof_alloc_node_t *)iallocztm(tsd_tsdn(tsd), sz,
			sz_size2index(sz), false, NULL, true, arena_get(TSDN_NULL, 0, true), true);
...
bool prof_log_start(tsdn_t *tsdn, const char *filename) {
	...
	if (!prof_log_atexit_called) {
		...
		if (atexit(prof_log_stop_final) != 0) {
			malloc_write("<jemalloc>: Error in atexit() " "for logging\n");
			if (opt_abort) {
				abort();
			}
			ret = true;
...
static void prof_log_emit_metadata(emitter_t *emitter) {
	...
	emitter_json_kv(emitter, "prof_time_resolution", emitter_type_string, &res_type);
...
bool prof_log_stop(tsdn_t *tsdn) {
	...
	if (prof_log_dummy) {
		...
		fd = creat64(log_filename, 0644);
	}

	if (fd == -1) {
		malloc_printf(
				"<jemalloc>: creat() for log file \"%s\" " " failed with %d\n", log_filename,
				errno);
		if (opt_abort) {
			abort();
		}
	...
	buf_writer_init(tsdn, &buf_writer, prof_emitter_write_cb, &arg, NULL, PROF_LOG_STOP_BUFSIZE);
	emitter_init(&emitter, emitter_output_json_compact, buf_writer_cb, &buf_writer);
