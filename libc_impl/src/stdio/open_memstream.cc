...
static off_t ms_seek(FILE *f, off_t off, int whence) {
	ssize_t base;
	struct memstream_cookie *c = (memstream_cookie *)f->cookie;
	if (whence > 2U) {
fail:
		errno = EINVAL;
		return -1;
...
static size_t ms_write(FILE *f, const unsigned char *buf, size_t len) {
	...
	if (len + c->pos >= c->space) {
		...
		newbuf = (char *)realloc(c->buf, len2);
		if (!newbuf) {
			return 0;
		}
		*c->bufp = c->buf = newbuf;
...
__SPRT_C_FUNC FILE *open_memstream(char **bufp, size_t *sizep) __SPRT_NOEXCEPT {
	struct ms_FILE *f;
	char *buf;

	if (!(f = (ms_FILE *)malloc(sizeof *f))) {
		return 0;
	}
