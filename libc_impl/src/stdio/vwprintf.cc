...
static const char sizeprefix['y' - 'a'] = {['a' - 'a'] = 'L',
	['e' - 'a'] = 'L',
	['f' - 'a'] = 'L',
	['g' - 'a'] = 'L',
	['d' - 'a'] = 'j',
...
static int wprintf_core(FILE *f, const wchar_t *fmt, va_list *ap, union arg *nl_arg, int *nl_type) {
	...
	for (;;) {
		...
		case 'S':
			...
			l = w;
			continue;
		case 'm': arg.p = strerror(errno);
		case 's':
			if (!arg.p) {
				arg.p = (void *)"(null)";
			}
			bs = (char *)arg.p;
			for (i = l = 0; l < (p < 0 ? INT_MAX : p) && (i = mbtowc(&wc, bs, MB_LEN_MAX)) > 0;
					bs += i, l++);
			if (i < 0) {
				return -1;
			}
		...
		case 'e':
		case 'f':
		case 'g': l = fprintf(f, charfmt, w, p, arg.f); break;
		case 'd':
		case 'i':
		case 'o':
		case 'u':
		case 'x':
		case 'p': l = fprintf(f, charfmt, w, p, arg.i); break;
		}
	}
...
__SPRT_C_FUNC int vfwprintf(FILE *__restrict f, const wchar_t *__restrict fmt,
...
struct vswprintf_cookie {
	wchar_t *ws;
	size_t l;
};

static size_t vswprintf_write(FILE *f, const unsigned char *s, size_t l) {
	size_t l0 = l;
	int i = 0;
	struct vswprintf_cookie *c = (vswprintf_cookie *)f->cookie;
	if (s != f->wbase && vswprintf_write(f, f->wbase, f->wpos - f->wbase) == -1) {
		return -1;
	}
	while (c->l && l && (i = mbtowc(c->ws, (const char *)s, l)) >= 0) {
		if (!i) {
			i = 1;
		}
		s += i;
...
__SPRT_C_FUNC int vswprintf(wchar_t *__restrict s, size_t n, const wchar_t *__restrict fmt,
		va_list ap) __SPRT_NOEXCEPT {
	int r;
	unsigned char buf[256];
	struct vswprintf_cookie c = {s, n - 1};
	FILE f = {
		.write = vswprintf_write,
		.buf = buf,
		.buf_size = sizeof buf,
		.lbf = EOF,
		.cookie = &c,
	...
	vswprintf_write(&f, 0, 0);
	return r >= n ? -1 : r;
}

__SPRT_C_FUNC int vwprintf(const wchar_t *__restrict fmt, va_list ap) __SPRT_NOEXCEPT {
	return vfwprintf(stdout, fmt, ap);
}

__SPRT_C_FUNC int wprintf(const wchar_t *__restrict fmt, ...) __SPRT_NOEXCEPT {
	int ret;
	va_list ap;
	va_start(ap, fmt);
	ret = vwprintf(fmt, ap);
