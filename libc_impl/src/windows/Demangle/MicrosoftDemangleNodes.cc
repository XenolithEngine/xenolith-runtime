...
static void outputSingleQualifier(OutputBuffer &OB, Qualifiers Q) {
	switch (Q) {
	case Q_Const: OB << "const"; break;
	case Q_Volatile: OB << "volatile"; break;
	case Q_Restrict: OB << "__restrict"; break;
	default: break;
	}
}

static bool outputQualifierIfPresent(OutputBuffer &OB, Qualifiers Q, Qualifiers Mask,
		bool NeedSpace) {
	if (!(Q & Mask)) {
		return NeedSpace;
	}
...
static void outputCallingConvention(OutputBuffer &OB, CallingConv CC) {
	...
	switch (CC) {
	case CallingConv::Cdecl: OB << "__cdecl"; break;
	case CallingConv::Fastcall: OB << "__fastcall"; break;
	case CallingConv::Pascal: OB << "__pascal"; break;
	case CallingConv::Regcall: OB << "__regcall"; break;
	case CallingConv::Stdcall: OB << "__stdcall"; break;
	case CallingConv::Thiscall: OB << "__thiscall"; break;
	case CallingConv::Eabi: OB << "__eabi"; break;
	case CallingConv::Vectorcall: OB << "__vectorcall"; break;
	case CallingConv::Clrcall: OB << "__clrcall"; break;
	case CallingConv::Swift: OB << "__attribute__((__swiftcall__)) "; break;
	case CallingConv::SwiftAsync: OB << "__attribute__((__swiftasynccall__)) "; break;
	default: break;
	}
}

sprt::__malloc_string Node::toString(OutputFlags Flags) const {
	OutputBuffer OB;
...
void PrimitiveTypeNode::outputPre(OutputBuffer &OB, OutputFlags Flags) const {
	switch (PrimKind) {
		...
		OUTPUT_ENUM_CLASS_VALUE(PrimitiveKind, Nullptr, "sprt::nullptr_t");
...
void NodeArrayNode::output(OutputBuffer &OB, OutputFlags Flags, sprt::StringView Separator) const {
	if (Count == 0) {
		return;
	}
	if (Nodes[0]) {
...
void EncodedStringLiteralNode::output(OutputBuffer &OB, OutputFlags Flags) const {
	switch (Char) {
	case CharKind::Wchar: OB << "L\""; break;
	case CharKind::Char: OB << "\""; break;
	case CharKind::Char16: OB << "u\""; break;
	case CharKind::Char32: OB << "U\""; break;
	}
	OB << DecodedString << "\"";
	if (IsTruncated) {
		OB << "...";
...
void TemplateParameterReferenceNode::output(OutputBuffer &OB, OutputFlags Flags) const {
	...
	for (int I = 1; I < ThunkOffsetCount; ++I) { OB << ", " << ThunkOffsets[I]; }
	if (ThunkOffsetCount > 0) {
		OB << "}";
	}
}
