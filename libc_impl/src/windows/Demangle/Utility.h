...
class OutputBuffer {
	...
	OutputBuffer &writeUnsigned(uint64_t N, bool isNeg = false) {
		...
		return operator+=(sprt::StringView(TempPtr, Temp.data() + Temp.size() - TempPtr));
	}
...
public:
	...
	operator sprt::StringView() const { return sprt::StringView(Buffer, CurrentPosition); }
	...
	unsigned CurrentPackIndex = sprt::Max<unsigned>;
	unsigned CurrentPackMax = sprt::Max<unsigned>;
	...
	OutputBuffer &operator+=(sprt::StringView R) {
		if (size_t Size = R.size()) {
			grow(Size);
			sprt::memcpy(Buffer + CurrentPosition, &*R.begin(), Size);
	...
	OutputBuffer &prepend(sprt::StringView R) {
		size_t Size = R.size();
		if (!Size) {
			return *this;
		}
		...
		grow(Size);
		sprt::memmove(Buffer + Size, Buffer, CurrentPosition);
		sprt::memcpy(Buffer, &*R.begin(), Size);
		CurrentPosition += Size;
	...
	OutputBuffer &operator<<(sprt::StringView R) { return (*this += R); }

	OutputBuffer &operator<<(char C) { return (*this += C); }

	OutputBuffer &operator<<(long long N) {
		return writeUnsigned(static_cast<unsigned long long>(sprt::abs(N)), N < 0);
	}
	...
	void insert(size_t Pos, const char *S, size_t N) {
		...
		grow(N);
		sprt::memmove(Buffer + Pos + N, Buffer + Pos, CurrentPosition - Pos);
		sprt::memcpy(Buffer + Pos, S, N);
		CurrentPosition += N;
...
public:
	...
	ScopedOverride(T &Loc_, T NewVal) : Loc(Loc_), Original(Loc_) { Loc_ = sprt::move(NewVal); }
	~ScopedOverride() { Loc = sprt::move(Original); }
